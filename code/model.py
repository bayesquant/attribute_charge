...
class LSTM_MODEL(object):
	...
	def __init__(self, word_embeddings, attr_table, config):
		...
		self.attn_weights = tf.concat( [tf.expand_dims(temp,1) for temp in self.attention_weights],1)
		self.attr_preds = tf.concat( [tf.expand_dims(temp,1) for temp in self.attn_attr_preds],1)
		self.attr_loss = tf.concat( [tf.expand_dims(temp,1) for temp in self.attn_attr_loss],1)
